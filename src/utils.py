...
def numpy_mask_to_qimage(mask_array, color=None):
    """将numpy掩码数组转换为QImage"""
    ...
    try:
        ...
        # 转换为QImage
        bytes_per_line = 4 * width
        qimg = QImage(result.data, width, height, bytes_per_line, QImage.Format_ARGB32)
        return qimg.copy()  # 创建副本以避免内存问题
        
    except Exception:
        return QImage()
        print(f"设置了 {pixels_set} 个像素")
